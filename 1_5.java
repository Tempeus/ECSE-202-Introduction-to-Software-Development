...
public class Bounce extends GraphicsProgram {
	...
	public void run() {
		...
		// movement mechanic of the ball (will be affected if changing the size of
		// canvas)
...
//					pause(intervalTime*1000);
					...
					h = initialUpPosition + vt * t - 0.5 * G * Math.pow(t, 2); // for some reason the value here is not
																			// changed from the if statement above
					println("gay");
				}
			}
...
/*
 * problems: []the ball isn't moving vertically []bouncing mechanic not
 * implemented
 */
